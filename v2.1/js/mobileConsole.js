...
var mobileConsole = (function () {
  ...
  function initConsoleElement() {
    ...
    function toggleScroll() {
      if (elements.scrollcontainer.scrollTop > elements.scrollcontainer.scrollHeight - elements.scrollcontainer.offsetHeight - 20) {
    ...
    function attach(console) {
      ...
      // disable scroll to top on attach
      // elements.scrollcontainer.scrollTop = elements.scrollcontainer.scrollHeight;
  ...
  function init(show = false) {
    if (!status.initialized) {
      if (consoleElement && mobileConsole) {
        ...
        setTimeout(function() {
          setCSS(elements.base, { opacity: 1 });
          if (!show) {
            setCSS(elements.base, { pointerEvents: 'auto' });
            setCSS(elements.base, { display: 'none' });
          }
        }, 500);
      }
    }
  ...
  function hide() {
    if (elements.base && status.initialized) {
      setCSS(elements.base, { display: 'none' });
    } else {
      init(false);
    }
  ...
  function show() {
    if (elements.base && status.initialized) {
      setCSS(elements.base, { display: 'block' });
    } else {
      init(true);
    }
  }
